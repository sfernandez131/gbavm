...
UBYTE VM_STEP(SCRIPT_CTX * THIS) {
    ...
    switch (op) {
        ...
        case 0x3E: { uint16_t *r = (uint16_t *)vm_resolve_ref(THIS, A_I16(0)); hw_actor_set_move_speed((INT16)r[0], A_U8(2)); break; }
        ...
        case 0x7C: { UBYTE mask = A_U8(0); UBYTE n = 0;
            for (UBYTE b = 0; b < 8; b++) if (mask & (1 << b)) n++;
            hw_load_palette(mask, A_U8(1), THIS->PC);
            THIS->PC += 8 * n;
            break; }
